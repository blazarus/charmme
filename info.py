...
#       print "after last loc", info['last_loc']
        ...
        # Just initialize these now - will be filled in later
        info['topics'] = None   
        info['known'] = True

        print "returning user info", info
        return info
...
def get_charms(email):
    charms = get_from_url('charms', 'http://tagnet.media.mit.edu/charms?user_name=' + email)
    if len(charms) == 0:
